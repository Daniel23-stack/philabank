...
Route::middleware(['install'])->group(function () use ($email_verification, $allow_signup) {
    ...
    Route::group(['middleware' => $email_verification == 'enabled' ? ['auth', 'verified'] : ['auth']], function () {
        ...
        Route::group(['middleware' => ['Email2FA']], function () {
            ...
            // Banking Layout Demo Route
            Route::get('banking-demo', function () {
                return view('banking-demo');
            })->name('banking.demo');
            ...
            Route::group(['middleware' => ['admin'], 'prefix' => 'admin'], function () {
                ...
                // Admin Dashboard
                Route::get('dashboard', 'DashboardController@admin_dashboard')->name('admin.dashboard');
                ...
                //Utility Controller
                Route::post('administration/update_kyc_form', 'UtilityController@update_kyc_form')->name('settings.update_kyc_form');
                ...
                // Route::resource('partners', 'PartnerController')->except('show'); // Commented out - PartnerController missing
            ...
            Route::group(['middleware' => ['permission'], 'prefix' => 'admin'], function () {
                ...
                //User Controller
                Route::get('users/get_user_transaction_data/{user_id}', 'UserController@get_user_transaction_data');
                Route::get('users/get_table_data/{status?}', 'UserController@get_table_data');
                Route::match(['get', 'post'], 'users/custom_fields', 'UserController@custom_fields')->name('users.custom_fields');
                ...
                Route::patch('transactions/{id}/update', 'TransactionController@update')->name('transactions.update');
                ...
                //FDR Controller
                Route::match(['get', 'post'], 'fixed_deposits/{id}/complete_before_maturity', 'FixedDepositController@complete_before_maturity')->name('fixed_deposits.complete_before_maturity');
                ...
                //DPS Plans
                Route::resource('dps_plans', 'DPSPlanController');
                Route::post('dps/get_table_data', 'DPSController@get_table_data');
                Route::get('dps/get_due_dps_data', 'DPSController@get_due_dps_data');
                Route::get('dps/due_dps', 'DPSController@due_dps')->name('dps.due_dps');
                Route::resource('dps', 'DPSController')->except('destroy');
            ...
            Route::group(['middleware' => ['customer']], function () {
                ...
                Route::get('transfer/get_account_name/{accountNumber}', 'Customer\TransferController@get_account_name')->name('transfer.get_account_name');
                ...
                //DPS Scheme
                Route::get('dps_schemes/plans', 'Customer\DPSController@plans')->name('dps_scheme.plans');
                Route::get('dps_schemes', 'Customer\DPSController@index')->name('dps_scheme.index');
                Route::match(['get', 'post'], 'dps_schemes/apply/{id}', 'Customer\DPSController@apply')->name('dps_scheme.apply');
                ...
                Route::get('tickets/mark_as_closed/{id}', 'Customer\SupportTicketController@mark_as_closed')->name('tickets.mark_as_closed');
    ...
    // Service Pages
    Route::get('/services/microloans', function () {
        return view('website.services.microloans');
    })->name('services.microloans');

    Route::get('/services/business-loans', function () {
        return view('website.services.business-loans');
    })->name('services.business-loans');

    Route::get('/services/personal-finance', function () {
        return view('website.services.personal-finance');
    })->name('services.personal-finance');

    Route::get('/services/test', function () {
        return view('website.services.test');
    })->name('services.test');

    // Public Banking Layout Demo (no authentication required)
    Route::get('/banking-demo', function () {
        return view('banking-demo');
    })->name('banking.demo.public');
