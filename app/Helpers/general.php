...
if (!function_exists('increment_account_number')) {
	function increment_account_number($default = '') {
		...
		update_option('next_account_number', strval($account_number));
		return $account_number;
	}
}
...
if (!function_exists('dps_status')) {
	function dps_status($status) {
		if ($status == 0) {
			return "<span class='badge badge-warning'>" . _lang('Pending') . "</span>";
		} else if ($status == 1) {
			return "<span class='badge badge-success'>" . _lang('Active') . "</span>";
		} else if ($status == 2) {
			return "<span class='badge badge-info'>" . _lang('Matured') . "</span>";
		} else if ($status == 3) {
			return "<span class='badge badge-dark'>" . _lang('Closed') . "</span>";
		}
	}
}
...
if (!function_exists('generate_input_field')) {
	function generate_input_field($field, $initialValue = null) {
		...
		$value = $initialValue == null ? old('requirements.' . $field_name) : $initialValue;

		$field_html = '';
		if ($field_type == 'text') {
			$field_html = '<input type="text" class="form-control" name="requirements[' . $field_name . ']" value="' . $value . '" placeholder="' . $field_label . '"' . $validation . '>';
		} elseif ($field_type == 'textarea') {
			$field_html = '<textarea class="form-control" name="requirements[' . $field_name . ']" placeholder="' . $field_label . '"' . $validation . '>' . $value . '</textarea>';
		} elseif ($field_type == 'number') {
			$field_html = '<input type="number" class="form-control" name="requirements[' . $field_name . ']" value="' . $value . '" placeholder="' . $field_label . '"' . $validation . '>';
			...
			$field_html = '<select class="form-control auto-select" name="requirements[' . $field_name . ']" data-selected="' . $value . '" data-placeholder="' . $field_label . '"' . $validation . '>' . $selectOptions . '</select>';
		} elseif ($field_type == 'file') {
			if ($initialValue == null) {
				$field_html = '<input type="file" class="trickycode-file" name="requirements[' . $field_name . ']" data-value="' . $value . '" data-placeholder="' . $field_label . '"' . $validation . '>';
			} else {
				$field_html = '<input type="file" class="trickycode-file" name="requirements[' . $field_name . ']" data-value="' . $value . '" data-placeholder="' . $field_label . '">';
			}
		}
		...
		if (!empty($custom_fields)) {
			foreach ($custom_fields as $field) {
				...
				if ($field->field_type == 'file') {
					$file_required = $field->validation;
					if ($edit == true) {
						$file_required = 'nullable';
					}
					$max_size = $field->max_size * 1024;
					$validationRules['requirements.' . $field->field_name] = $file_required . "|mimes:jpeg,jpg,png,pdf|max:$max_size";
				}

				if ($field->validation == 'required') {
					$validationMessages['requirements.' . $field->field_name . '.required'] = 'The ' . $field->field_name . ' is required';
				}

				if ($field->field_type == 'file') {
					$validationMessages['requirements.' . $field->field_name . '.mimes'] = 'The ' . $field->field_name . ' must be a file of type: jpeg, jpg, png, pdf';
					$validationMessages['requirements.' . $field->field_name . '.max'] = 'The ' . $field->field_name . ' may not be greater than ' . $field->max_size . ' MB';
				}
			}
		}
